      ...
      <div className="mb-6 grid gap-2">
        {INCIDENT_SCENARIOS.map((item) => (
          <button
            ...
            className={cn(
              "min-h-11 w-full border px-3 py-2 text-left font-mono text-[11px] tracking-[0.08em]",
              item.id === scenario.id
          ...
          >
            ...
            <span className="mt-0.5 block text-sm normal-case tracking-normal">
              {item.title}
            </span>
          </button>
        ))}
  ...
  return (
    <div>
      <header className="border border-border p-4 sm:p-5">
        ...
        <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.12em] text-muted [overflow-wrap:anywhere]">
          {scenario.category} · {scenario.environment}
        </p>
        <h2 className="mt-3 font-display text-2xl tracking-tight break-words text-foreground sm:text-3xl">
          {scenario.title}
        </h2>
        ...
        <section className="min-h-52 border border-border p-4" aria-live="polite">
          {channel ? (
            <>
              ...
              <pre className="mt-3 max-w-full overflow-x-auto whitespace-pre-wrap break-words font-mono text-xs leading-relaxed text-muted">
                {channel.lines.join("\n")}
              </pre>
            </>
          ) : (
