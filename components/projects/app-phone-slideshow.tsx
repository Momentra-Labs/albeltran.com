...
function coverflowPose(offset: number, compact: boolean, reduce: boolean) {
  ...
  return {
    x: `${offset * (compact ? 26 : 34)}%`,
    y: 0,
    rotateY: offset * -28,
    scale: offset === 0 ? 1 : Math.max(0.8, 0.95 - abs * 0.05),
...
export function AppPhoneShowcase({
  ...
  return (
    <div
      className={cn("device-phone-showcase", !compact && "device-phone-showcase-hero")}
      role={compact ? undefined : "region"}
      aria-roledescription={compact ? undefined : "carousel"}
      aria-label={compact ? undefined : `${project.name} screens`}
      ...
      <div className="device-phone-coverflow-wrap">
        ...
        >
          {visible.map((i) => {
            ...
            const abs = Math.abs(offset);
            const interactive = !compact && !reduce && abs > 0 && abs <= 2;
            return (
              <motion.div
                key={screen.kicker}
        ...
        {!compact ? (
          <>
            <button
              ...
              className="device-phone-coverflow-nav device-phone-coverflow-prev"
              onClick={() => go(index - 1)}
            >
              ‹
            </button>
            <button
              ...
              className="device-phone-coverflow-nav device-phone-coverflow-next"
              onClick={() => go(index + 1)}
            >
              ›
            </button>
